...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (match) {
            //per login funzionante togliere commento if (match) {
            //login
            console.log("match")
            const token = jwt.sign({
                email: user.email,
...
app.get('/tickets', checkAuth, async (req, res) => {
    try {
        ...
        const tickets = await Ticket.find({}).limit(500).sort({ _id: -1 }).populate('customer_id').populate('operator_id')
        if (!tickets) {
...
app.post('/tickets', checkAuth, upload.array('images[]', 10), async (req, res) => {
    ...
    if (req.body.ticket_time == "") {
        ...
        //   console.log(JSON.parse(JSON.stringify(req.body.ticket_time)))
        console.log(req.body.ticket_time.length)

        // if(parsed_ticket_time.length>1){
        ...
        console.log(newTicket.ticket_time);
        console.log(newTicket.ticket_time.length);
    }
    //return;
    ...
    try {
        ...
        //console.log( "parsed_ticket_time "+newTicket.ticket_time );
        newTicket.ticket_time = parsed_ticket_time;
...
//=> Ottiene Ticket Di Specifico Tecnico (admin)
app.get('/ticketsbytechAdmin/:techId', checkAuth, async (req, res) => {
    try {
        const allTickets = await Ticket.find({}).populate("added_by customer_id").sort({ _id: -1 })
        if (!allTickets) {
            throw Error("Ticket non Trovato")
        }
        const ticketByTech = allTickets.filter(ticket => ticket.operator_id != null && ticket.operator_id == req.params.techId);
        res.status(200).json(ticketByTech);
    } catch (error) {
        res.status(500).json({
            error: error
...
//=> Ottiene Ticket Di specifico cliente
app.get('/ticketsbycust/:custId', checkAuth, async (req, res) => {
    console.log("enterrrrrr")
    console.log(req.params.custId)
    try {
        const allTickets = await Ticket.find({}).populate("customer_id").sort({ _id: -1 })
        if (!allTickets) {
            throw Error("Ticket non Trovato")
        }
        const ticketByCust = allTickets.filter(ticket => ticket.customer_id != null && ticket.customer_id._id == req.params.custId);
        //  console.log(ticketByCust)
        res.status(200).json(ticketByCust);
    } catch (error) {
        res.status(500).json({
            error: error
        })
    ...
    try {
        ...
        if (req.body.password) {
            bcrypt.hash(req.body.password, 10, async (err, hash) => {
                ...
                userToUpdate.status_holiday = req.body.status_holiday;
                await userToUpdate.save();

                console.log(req.body.status_holiday)
                // res.status(200).json({
                //     success : true
                // });
...
app.get("/customers", checkAuth, async (req, res) => {
    try {
        const customersToFind = await Customer.find({ customer_disabled: false }).populate("tickets");
...
//=> Ottiene tutti i clienti Disattivati
app.get("/customersDeactivated", checkAuth, async (req, res) => {
    try {
        const customersToFind = await Customer.find({ customer_disabled: true }).populate("tickets");
        //const customersToFind = await Customer.find({});
...
app.get("/customer/:customerId", checkAuth, async (req, res) => {
    try {
        const customerToFind = await Customer.findById({ _id: req.params.customerId }).populate({ path: "tickets", populate: ("operator_id") })
        if (!customerToFind) {
...
// => Disabilita specifico cliente
app.post("/disableCustomer", checkAuth, async (req, res) => {
    try {
        const customerToDisable = await Customer.findByIdAndUpdate({ _id: req.body.customerId }, { customer_disabled: true })

        if (!customerToDisable) {
            throw Error("Non è stato possibile disabilitare il cliente");
        }
        res.status(200).json({
            success: true
        })
...
// => Riabilita specifico cliente
app.post("/ableCustomer", checkAuth, async (req, res) => {
    try {
        const customerToDisable = await Customer.findByIdAndUpdate({ _id: req.body.customerId }, { customer_disabled: false })
...
app.get('/customers/search/:term', checkAuth, async (req, res) => {
    try {
        const customerToFind = await Customer.find({ company_name: { $regex: ".*" + req.params.term + ".*", $options: "i" } }).populate("tickets")
        if (!customerToFind) {
            throw Error("Non è stato possibile trovare il cliente");
        }
        res.status(200).json(customerToFind)
